...
class RotaryEmbeddingESM(torch.nn.Module):
    ...
    def __init__(
        ...
        if self.ext_factors:
            ext_factors = torch.tensor(self.ext_factors, dtype=torch.float32, device=device)
            inv_freq = 1.0 / (
                ext_factors * base ** (torch.arange(0, dim, 2, device=device, dtype=torch.float32) / dim)
            )
        else:
