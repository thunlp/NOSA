...
class GreedySearch:
    ...
    def _decode(self, input_ids, max_length=100, extra_end_token_ids=[], chunk_size: int = 4096, output=False, benchmark=False):
        if input_ids.dim() == 1:
        ...
        if benchmark:
            torch.cuda.synchronize()
            end_t = time.time()
            return self.tokenizer.batch_decode(input_ids[:, length:]), end_t - beg_t
        else:
