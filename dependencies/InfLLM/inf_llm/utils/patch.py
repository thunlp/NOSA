...
def patch_hf(
    ...
    **kwargs
    ...
    rope = RotaryEmbeddingESM(
        ...
        base,
        distance_scale,
        # model.model.layers[0].self_attn.rotary_emb.short_factor if hasattr(model.model.layers[0].self_attn.rotary_emb, "short_factor") else None,
        model.config.rope_scaling['short_factor'],
        model.device
    )
    ...
    def set_forward(m):
        if isinstance(m, Attention):
            if "q_proj" not in m.__dict__["_modules"]:
                query_pos = m.num_heads * m.head_dim
                key_value_pos = query_pos // m.num_key_value_groups
                m.q_proj = torch.nn.Linear(
                    m.qkv_proj.in_features,
                    query_pos,
                    device=m.qkv_proj.weight.device,
                    dtype=m.qkv_proj.weight.dtype,
                    bias=False,
                )
                m.k_proj = torch.nn.Linear(
                    m.qkv_proj.in_features,
                    key_value_pos,
                    device=m.qkv_proj.weight.device,
                    dtype=m.qkv_proj.weight.dtype,
                ...
                with torch.no_grad():
                    m.q_proj.weight.copy_(m.qkv_proj.weight[:query_pos, :])
                    m.k_proj.weight.copy_(
                        m.qkv_proj.weight[query_pos : query_pos + key_value_pos, :]
                    )
                    m.v_proj.weight.copy_(
                        m.qkv_proj.weight[query_pos + key_value_pos :, :]
                    )
