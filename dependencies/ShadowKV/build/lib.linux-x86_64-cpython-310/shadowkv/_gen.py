...
class ShadowKVCache_GEN:
    ...
    def register_k_landmark_decode(self, k_landmark, k_landmark_idx, layer_idx):
        ...
        self.k_landmark[layer_idx, :, :, self.current_landmark_num - num_landmarks:self.current_landmark_num, :].copy_(k_landmark.contiguous())
        self.k_landmark_idx[layer_idx, :, :, self.current_landmark_num - num_landmarks:self.current_landmark_num].copy_(k_landmark_idx.contiguous())
    ...
    def prefill_kv_cache(self,
        ...
        mask.scatter_(dim=-1, index=outlier_chunk_idx, value=False) # outlier_chunk_idx: (B, H, *)
        rest_idx = all_idx.masked_select(mask).view(self.batch_size, self.num_key_value_heads, -1) # 去掉了outliers和sliding window的所有idx
        # register rest_idxed landmarks to k_landmark
        self.register_k_landmark(landmark_candidates.gather(dim=2, index=rest_idx.unsqueeze(-1).expand(-1, -1, -1, self.head_dim)).view(self.batch_size, self.num_key_value_heads, -1, self.head_dim), rest_idx, layer_idx) # 第一个参数：(B, H, *, D), rest_idx: (B, H, *, D)
    ...
    def update_kv_cache(self, 
        ...
        #     new_tail = torch.cat((old_window_k, new_gen_k), dim=-2)
        #     self.k_cache_buffer[layer_idx][:, :, :self.prefill_local].copy_(new_tail[:, :, self.chunk_size:], non_blocking=True)
        #     new_landmark = new_tail[:, :, :self.chunk_size].mean(dim=-2, keepdim=True)
