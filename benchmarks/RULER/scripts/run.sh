...
ROOT_DIR="/somepath/sparse_train/sparse_train/test/RULER/generated" # the path that stores generated task samples and model predictions.
MODEL_DIR="/somepath" # the path that contains individual model folders from HUggingface.
...
echo "TASKS=${TASKS}"
if [ -z "${TASKS}" ]; then
    echo "Benchmark: synthetic is not supported"
    exit 1
fi
...
for MAX_SEQ_LENGTH in "${SEQ_LENGTHS[@]}"; do
    ...
    RESULTS_DIR="${ROOT_DIR}/${MODEL_NAME}/synthetic/${MAX_SEQ_LENGTH}"
